...
- **Account economy sync BUILT (devlog 0046):** signed-in progress saves to Supabase `player_economy` (JSONB blob, own-row RLS) via `economy-sync.ts`; profile button now auth-aware. **⚠️ OWNER must run `supabase/migrations/0002_player_economy.sql`** for saving to work (until then it no-ops). Account SYNC only — NOT trade-safe (trading needs server-authoritative, p2p-trading-epic P1).
