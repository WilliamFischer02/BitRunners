...
- **⚠️ DEPLOY STATE (read this):** PR #33 merged **only the first commit** (`b6d34fb`, multiplayer) to `main` on 2026-05-16 then closed. So prod `bitrunners.app` has **multiplayer only**. The services guide + entire Data Scrape mini-game (devlogs 0032–0037) are **5+ commits stranded on the branch**, never PR'd, never deployed. A **new PR** is opened for them. Merging it = **Cloudflare Pages prod deploy only (no Fly — no server/packages changes in the stranded set)**; owner-gated, not done.
...
- **CI status:** local gates green — `pnpm lint` clean (43 files), `pnpm typecheck` 8/8, `pnpm build` 5/5. No test suite (`vitest run` exits 1 on "no tests" — pre-existing, not a regression).
