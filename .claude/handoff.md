...
- **Active plan:** 5-chunk sprint in `docs/devlog/0039`. **A merged (#36); B+C merged (#37); D = backend-epic plan only (#38, `docs/design/p2p-trading-epic.md`, devlog 0042 — deferred until auth); E done (devlog 0043).** Sprint core complete. **Trading epic** is the next big body of work, gated on the owner's Supabase setup (env + migrations); its buildable-now pieces are the **email/password-only auth-UI redesign** + **room-code join** (see the epic plan's phasing P0a/P0b).
