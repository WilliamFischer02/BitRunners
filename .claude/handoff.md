...
- **Admin console phase 1 BUILT (devlog 0047):** `profiles.role` + `is_admin()` RLS helpers + `app_config` (migration **0003** — owner runs it, then sets own role to `admin` via SQL). `AdminConsole` (admin-only launcher), under-construction switch with dev-bypass (`ConstructionGate`, fails OPEN), account sync-status row. Next admin phases: dialogue editor, user table + token/credit grants, activity stats (each needs its own admin-RLS migration; grants need server-authoritative economy).
