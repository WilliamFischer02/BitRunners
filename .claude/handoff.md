...
- **Active plan:** 5-chunk sprint in `docs/devlog/0039`. **A merged (#36); B+C merged (#37).** **Chunk D = P2P trading → reclassified as a backend epic, deferred** (plan in `docs/design/p2p-trading-epic.md`, devlog 0042) — hard-blocked on live auth + a server-authoritative economy; no code shipped. **Chunk E (tutorial + server_speaker unlock) is next** (no backend dep). Trading resumes per the epic plan once auth lands.
